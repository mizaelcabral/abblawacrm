...
export async function POST(
  ...
  try {
    ...
    // 2) Find account owner user_id directly from accounts table
    ...
    const ownerUserId = accountRow?.owner_user_id;
    ...
    if (ownerUserId && (name || email || phone || !contactId)) {
      ...
      // Re-link conversation if existingSession's contact_id differs from newly resolved contactId
      if (conversationId && existingSession?.contact_id && contactId && contactId !== existingSession.contact_id) {
        await supabase
          .from('conversations')
    ...
    if (!session) {
      const { data: newSession } = await supabase
        ...
        .insert({
          ...
          visitor_email: email ? email.trim().toLowerCase() : null,
      ...
      if (email) updatePayload.visitor_email = email.trim().toLowerCase();
