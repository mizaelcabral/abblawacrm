...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  ...
  { value: 'agent', label: 'Agente', hint: 'Usa recursos; sem acesso a configurações' },
  { value: 'viewer', label: 'Visualizador', hint: 'Acesso apenas leitura em todo o app' },
];
...
export function MembersTab() {
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      toast.success(`Atualizado ${member.full_name || 'membro'} para ${nextRole}`);
    } catch (err) {
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              Por motivos de segurança, a URL de convite em texto simples é exibida apenas
              uma vez na criação — para compartilhar novamente, revogue o convite abaixo
              e crie um novo.
            </p>
          ) : null}
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              ...
              da conta? Eles serão desconectados desta conta e receberão uma nova
              conta pessoal no próximo login. O login deles não será excluído.
            </DialogDescription>
          </DialogHeader>
