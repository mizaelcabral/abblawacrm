...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  create_task: { label: "Criar Tarefa", icon: CheckSquare, border: "border-l-primary" },
  ai_respond: { label: "Resposta por IA (Gemini)", icon: Sparkles, border: "border-l-indigo-500" },
}
...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; label: string; hint: string }[] = [
  { value: "new_message_received", label: "Nova Mensagem Recebida", hint: "Qualquer mensagem recebida" },
  {
    ...
    hint: "Primeira vez que o contato envia mensagem (funciona para contatos adicionados manualmente também)",
  },
    ...
    <div className="space-y-2">
      <div>
        ...
        <Input
          ...
          placeholder="ex: preço, teste grátis, falar com vendas"
          className="bg-muted text-foreground"
        />
      </div>
      <div>
...
function StepEditor({
  ...
  switch (step.step_type) {
    ...
    case "create_task":
      return (
        <>
          <FieldBlock label="Título da Tarefa">
            <Input
              value={(cfg.title as string) ?? ""}
              onChange={(e) => set({ title: e.target.value })}
              className="bg-muted text-foreground"
          ...
          <FieldBlock label="Dias até o vencimento (0 para nenhum)">
            <Input
              type="number"
...
function previewFor(step: BuilderStep): string {
  switch (step.step_type) {
    ...
    case "wait": {
      ...
      const unitLabel = unit === "minutes" ? "minutos" : unit === "hours" ? "horas" : unit === "days" ? "dias" : unit
      return `${step.step_config.amount ?? "?"} ${unitLabel ?? ""}`
    }
    case "condition": {
      const subj = step.step_config.subject as string
      const subjLabel = subj === "tag_presence" ? "marcador" : subj === "contact_field" ? "campo" : subj === "message_content" ? "conteúdo" : subj === "time_of_day" ? "hora" : subj
      return `quando ${subjLabel ?? "?"}`
    }
    case "send_webhook":
      return (step.step_config.url as string) || "sem URL"
    case "create_task":
      return (step.step_config.title as string) || "nova tarefa"
    case "ai_respond":
      return (step.step_config.prompt as string) || "resposta da IA"
    default:
      return ""
  }
}
