      ...
      {tagNames.map((name) => {
        ...
        return (
          <span
            ...
            title={isKnown ? name : `${name} (será criado na importação)`}
          >
            <span
...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(
        'Nenhuma linha válida encontrada. Certifique-se de que o CSV possui uma coluna de cabeçalho "phone".'
      );
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (tagsAssigned > 0) {
        toast.success(
          `${tagsAssigned} tag${tagsAssigned !== 1 ? 's' : ''} atribuída${tagsAssigned !== 1 ? 's' : ''}`
        );
      }
        ...
        <div className="shrink-0 space-y-4 border-b border-border/80 px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            ...
            <DialogDescription className="leading-relaxed text-muted-foreground">
              Envie um CSV contendo uma coluna obrigatória{' '}
            ...
            {file ? (
              <>
                ...
                <span className="rounded-full bg-muted px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground">
                  {parsedRows.length} linha{parsedRows.length !== 1 ? 's' : ''} pronta{parsedRows.length !== 1 ? 's' : ''}
                </span>
              </>
            ) : (
              <>
            ...
            <div className="space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {tagStats.rowsWithTags > 0 && (
                    <span className="inline-flex items-center gap-1 rounded-md bg-muted/90 px-2 py-0.5 text-[11px] text-muted-foreground">
                      ...
                      {tagStats.rowsWithTags} contato{tagStats.rowsWithTags !== 1 ? 's' : ''}
                    </span>
                  )}
                </div>
              </div>
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-center text-[11px] text-muted-foreground">
                  + mais {parsedRows.length - PREVIEW_LIMIT} linha{parsedRows.length - PREVIEW_LIMIT !== 1 ? 's' : ''} não exibida{parsedRows.length - PREVIEW_LIMIT !== 1 ? 's' : ''}
                </p>
              )}
            </div>
          )}
              ...
              <div className="mt-3 flex flex-wrap gap-3">
                ...
                {result.skipped > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-amber-400">
                    ...
                    {result.skipped} duplicada{result.skipped !== 1 ? 's' : ''} ignorada{result.skipped !== 1 ? 's' : ''}
                  </div>
                )}
                {result.failed > 0 && (
